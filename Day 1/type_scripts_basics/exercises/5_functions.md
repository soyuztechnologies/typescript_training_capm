...
## Complete Function Patterns

### Block 8: Combining All Techniques

```typescript
// A function combining optional, default, destructured, and rest parameters
interface ApiConfig {
  baseUrl?: string;
  timeout?: number;
  retries?: number;
...
function fetchData(
  endpoint: string,
  { baseUrl = "https://api.example.com", timeout = 5000, retries = 3 }: ApiConfig = {},
  ...headers: string[]
) {
...
fetchData("/users", { baseUrl: "https://custom-api.com" }, "Authorization: Bearer token");

fetchData("/users", {}, "X-Custom-Header: value1", "X-Another-Header: value2");
```
...
### Block 9: Real-World Function Example

```typescript
// Configuration for a data processing function
...
// With callbacks
processArray(numbers, { limit: 3 }, 
  (result) => console.log("First callback:", result),
  (result) => console.log("Second callback:", result)
);
```
